#creating random data to import to mysql...
#later found out that using connector works better!

import csv, math, random

lines = 300000
show_percentage = 10
file_header = ["line","username","pw","user_comment","rand_no_1","rand_no_2"]
print("Creating randow data...")

with open('randomdata.csv', 'w', newline="", encoding='utf-8') as random_data_file:
    random_data_writer_object = csv.writer(random_data_file,delimiter=',')

    random_data_writer_object.writerow(file_header)

    for line in range(1,lines+1):
        username = str(line) +"user_abcd" + str(line)
        pw = str(line) + "pw_efgh" + str(line)
        user_comment = str(line) + "comment_ijkl" + str(line)
        rand_no_1 = random.randrange(0,10000)
        rand_no_2 = random.randrange(0,1000000)
        #print((line,username,pw,user_comment,rand_no_1, rand_no_2))
        random_data_writer_object.writerow((line,username,pw,user_comment,rand_no_1, rand_no_2))
        #print(post["title"]+","+str(post["score"])+","+post["time"])
        if line % int(lines*show_percentage/100) == 0:
            print(int(line*100/lines), "percent complete.")

print("Saved to randomdata.csv.")
