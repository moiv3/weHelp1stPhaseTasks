...
@app.get("/update")
async def update_page(request: Request, post_id: int | None):
    #fix no post id case
    if post_id == None:
        print("no post id found")
        return RedirectResponse("/error?message=One or more required fields is/are blank!", status_code=status.HTTP_303_SEE_OTHER)
    else:
        print("post id: found", post_id)
        website_db = mysql.connector.connect(host=db_host,user=db_user,password=db_pw,database=db_database)
        #sql queries
        website_db_cursor = website_db.cursor()
        #member id & message id for task 6
        cmd = "SELECT member.name, message.content, member.id, message.id FROM message JOIN member ON message.member_id = member.id WHERE message.id = %s"
        website_db_cursor.execute(cmd,(post_id,))
        website_db_result = website_db_cursor.fetchone()
        print(website_db_result)
        message_member_id = website_db_result[2]

        if website_db_result == None:
            print("Not found post id:", post_id, ", redirecting...")
            return RedirectResponse("/error?message=Post ID not found.", status_code=status.HTTP_303_SEE_OTHER)
        elif request.session["member_id"] != message_member_id:
            print("member id unmatch. member id of post:", message_member_id, "member id of sign-in state:", request.session["member_id"])
            return RedirectResponse("/error?message=Incorrect login.", status_code=status.HTTP_303_SEE_OTHER)
        else:
            print("Found post id:", post_id)
            return templates.TemplateResponse("updatemessage.html", {"request": request, "message": website_db_result})

@app.post("/updateMessage")
#update existing message
async def update_message(request: Request, updated_message_name: str | None = Form(None), message_id: int | None = Form(None)):
    #TODO: check login again!
    website_db = mysql.connector.connect(host=db_host,user=db_user,password=db_pw,database=db_database)
    website_db_cursor = website_db.cursor()
    print(updated_message_name, message_id)
    cmd = "UPDATE message SET content = %s WHERE id = %s"
    website_db_cursor.execute(cmd, (updated_message_name, message_id))
    website_db.commit()
    print("Modified message!")
    return RedirectResponse("/member", status_code=status.HTTP_303_SEE_OTHER)
    
"""
        return templates.TemplateResponse("updatemessage.html", {"request": request,
                                                                "member_id": request.session["member_id"],
                                                                "member_name": request.session["name"],
                                                                "messages": website_db_result})

    #return templates.TemplateResponse("updatemessage.html", {"request": request, "message": message})
